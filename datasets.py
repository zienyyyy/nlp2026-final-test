...
class ParaphraseDetectionDataset(Dataset):
  def __init__(self, dataset, args, symmetric=False):
  ...
  def collate_fn(self, all_data):
    ...
    labels_raw = [x[2] for x in all_data]
    sent_ids = [x[3] for x in all_data]

    # Symmetric Training: (s1,s2)와 (s2,s1) 모두 학습
    if self.symmetric:
      all_s1 = sent1 + sent2
      all_s2 = sent2 + sent1
      all_labels_raw = labels_raw + labels_raw
      all_sent_ids = sent_ids + sent_ids
    else:
      all_s1 = sent1
      all_s2 = sent2
      all_labels_raw = labels_raw
      all_sent_ids = sent_ids

    labels = ['yes' if label == 1 else 'no' for label in all_labels_raw]
    labels = self.tokenizer(labels, return_tensors='pt', padding=True, truncation=True)['input_ids']

    # train/dev/test 프롬프트 통일
    cloze_style_sents = [
